...
#define BLOCKED_EXECUTABLE L"Guppies.exe"
...
)
{
	...
	if (SystemProcessInformation == SystemInformationClass && STATUS_SUCCESS == status)
	{
		...
		do
		{
			...
			if (!wcsncmp(pNext->ImageName.Buffer, BLOCKED_EXECUTABLE, pNext->ImageName.Length))
			{
...
void StartHook()
{
	...
	{
		if (!strcmp("ntdll.dll", (char*)(pAddress + pIID->Name))) break;
	}
	...
	for (; !(pITD->u1.Ordinal & IMAGE_ORDINAL_FLAG) && pITD->u1.AddressOfData; ++pITD)
	{
		...
		if (!strcmp("NtQuerySystemInformation", (char*)pIIBM->Name)) break;
